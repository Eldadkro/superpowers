...
#
# Claude Code compatibility check: tests whether a skill is invoked when the user explicitly requests it by name
...
# Run the Claude Code compatibility harness with isolated environment
...
# Check if the Claude Code harness took action BEFORE invoking the skill (the failure mode)
echo ""
echo "Checking for premature action..."

# Look for tool invocations before the Skill invocation
# This detects the failure mode where the harness starts doing work without loading the skill
...
if [ -n "$FIRST_SKILL_LINE" ]; then
    ...
    if [ -n "$PREMATURE_TOOLS" ]; then
        ...
        echo ""
        echo "This indicates the harness started working before loading the requested skill."
    else
